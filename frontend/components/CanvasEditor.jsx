...
const showNotice = (message, type = 'info') => { setNotice(message); setNoticeType(type); };
  ...
  const authenticate = async (mode) => {
    try {
      ...
      showNotice(mode === 'register' ? 'Account created successfully — you are signed in.' : 'Signed in successfully.', 'success');
    } catch (error) {
      const isDuplicateEmail = mode === 'register' && /already registered/i.test(error.message);
      showNotice(isDuplicateEmail ? 'This email is already registered. Please sign in.' : error.message, 'error');
    }
  };
        ...
        <aside className="left-panel">
          ...
          {!token && <>
            ...
            <div className="split-actions">
              <button onClick={() => authenticate('login')} disabled={!canAuthenticate}>Sign in</button>
              <button onClick={() => authenticate('register')} disabled={!canAuthenticate}>Register</button>
            </div>
          </>}
          {token && <>
  <div className="account-status"><span className="status-dot" aria-hidden="true" />Signed in</div>
  <button onClick={() => { localStorage.removeItem('canvas-token'); setToken(''); setSavedCanvases([]); showNotice('Signed out successfully.'); }}>Sign out</button>
</>}
          <hr />
          ...
          <div className="saved-list">
            {savedCanvases.map((item) => (
              <div className="saved-item" key={item._id}>
                <button onClick={() => openCanvas(item._id)}>{item.name}</button>
                <button aria-label={`Delete ${item.name}`} onClick={() => removeCanvas(item._id)}>×</button>
              </div>
            ))}
        ...
        <section className="editor">
          <div className="canvas-title">
            <input value={canvas.name} aria-label="Canvas name" onChange={(e) => commitCanvas({ ...canvas, name: e.target.value })} />
           {/* <span className={`notice notice-${noticeType}`}>{token && notice === 'Create an account or sign in to save your work.' ? 'Signed in — changes autosave' : notice}</span> */}


           <span className={`notice ${noticeType === 'error' ? 'notice-error' : token ? 'notice-success' : ''}`}>{token && notice === 'Create an account or sign in to save your work.' ? 'Signed in — changes autosave' : notice}</span>
          </div>
        ...
        <aside className="right-panel">
          ...
          {selected && <>
            ...
            <div className="number-pair">
              <label>X<input aria-label="X position" type="number" value={selected.x} onChange={(e) => updateSelected('x', e.target.value)} /></label>
              <label>Y<input aria-label="Y position" type="number" value={selected.y} onChange={(e) => updateSelected('y', e.target.value)} /></label>
            </div>
            {selected.type === 'circle'
              ? <Field label="Radius" type="number" value={selected.radius} onChange={(value) => updateSelected('radius', value)} />
              : <>
                  <Field label="Width" type="number" value={selected.width} onChange={(value) => updateSelected('width', value)} />
                  <Field label="Height" type="number" value={selected.height} onChange={(value) => updateSelected('height', value)} />
                </>}
            ...
            {selected.type === 'text' && <>
              ...
              <Field label="Font size" type="number" value={selected.fontSize || 24} onChange={(value) => updateSelected('fontSize', value)} />
            </>}
            <div className="split-actions">
              <button onClick={() => moveLayer(-1)}>Move back</button>
              <button onClick={() => moveLayer(1)}>Move forward</button>
            </div>
