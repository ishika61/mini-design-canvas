      ...
      <section className="workspace">
        <aside className="left-panel">
          ...
          {token && <>
            ...
            <button onClick={() => { localStorage.removeItem('canvas-token'); setToken(''); setSavedCanvases([]); }}>Sign out</button>
          </>}
          <hr />
          <h2>Saved canvases</h2>
          <div className="saved-list">
        ...
        <section className="editor">
          <div className="canvas-title">
           ...
           <span>{token && notice === 'Create an account or sign in to save your work.' ? 'Signed in — changes autosave' : notice}</span>
          </div>
          <DesignStage elements={canvas.elements} selectedId={selectedId} onSelect={setSelectedId} onChange={updateElement} onStageReady={setStage} />
        </section>
